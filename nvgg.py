...
from modules import NLinear, NConv2d, NModel
...
class VGG(NModel):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            NLinear(512 * 3 * 3, 4096),
            nn.ReLU(),
            NLinear(4096, 4096),
            nn.ReLU(),
            NLinear(4096, num_classes),
        )
        if init_weights:
...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            v = cast(int, v)
            conv2d = NConv2d(in_channels, v, kernel_size=3, padding=1)
            if batch_norm:
...
def _vgg(arch: str, cfg: str, batch_norm: bool, pretrained: bool, progress: bool, **kwargs: Any) -> VGG:
    ...
    N = 16
    model = VGG(make_layers(cfgs[cfg], batch_norm=batch_norm), N=N, **kwargs)
    # if pretrained:
